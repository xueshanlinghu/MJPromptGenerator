...
编辑 `src/config/prompts.yaml` 文件，支持一级和二级分类结构：
...
编辑 `src/config/parameters.yaml` 文件添加 Midjourney 参数：
...
开发环境下，修改 YAML 配置文件后会自动重新加载，无需刷新浏览器：
- 保存配置文件后，控制台会显示 `🔄 Reloading config...`
- 界面会自动更新，显示最新的配置内容
